...
**Second prerequisite, after Python: the GitHub CLI.** `gh --version` and `gh auth status` must both succeed; the procedure's Step 0.7 says what to offer where either fails, and what a refusal costs — the project will not receive method updates.
