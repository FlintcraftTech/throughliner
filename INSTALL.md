...
**Already have Claude Code and a paid plan?** You can skip the opening questions below. Still do the quick app check in [Step 1 — Opening interview](#step-1--opening-interview) to confirm you're actually in Claude Code (not the Claude chat app), then jump straight to [Branch B — Install the Throughliner plugin](#branch-b--install-the-throughliner-plugin). Branch B checks the prerequisites, then has you paste two install commands into a terminal.
...
### B.1 — Check the prerequisites, then run the two install commands

**The prerequisites, each with its own check.** Have the user open a terminal — on Windows, PowerShell; on a Mac, Terminal — and run each check before going on.

- **Python 3, installed and on the path.** The plugin's safety checks and its session-opening facts are small Python scripts; without Python they silently do nothing, and the plugin reports success anyway. The check is `python --version`, and it must print a version number. The plugin's hooks reach `py` where Windows has it — the launcher the python.org installer adds — and `python` otherwise, so on Windows `py --version` printing a version is enough even where `python` is not the real interpreter. On a fresh Windows machine it may instead print "Python was not found; run without arguments to install from the Microsoft Store" — that is a placeholder, not Python. Install Python from python.org (ticking "Add python.exe to PATH" in the installer), close and reopen the terminal, and check again.
...
- **Git.** The method commits at every session close, so git must be installed — a machine passing the three checks above but lacking git finds out at its first close, after the install was declared successful. The check is `git --version`, and it must print a version number. If it does not: on Windows install Git for Windows from https://gitforwindows.org, which also supplies the Bash the plugin's docs assume; on a Mac run `xcode-select --install`, or install git with Homebrew; on Linux use the distribution's package manager (`apt install git`, `dnf install git`). Then close and reopen the terminal and check again.
