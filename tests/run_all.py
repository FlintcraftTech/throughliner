...
# This file sits at <project root>/tests/, so the root is one level up. Every
# suite computes its own root the same way, by walking a fixed number of
# levels from its own location.
...
def assert_footing():
    ...
    print("run_all: this file walks one folder up from its own location, so "
